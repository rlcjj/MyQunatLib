...
class SortStocksByFactors(object):
    ...
    def __init__(self,finRptDataAddr,mktDataAddr):
        """Constructor"""
        self.getFctVal = GetVal.CalcFactorVals(finRptDataAddr,mktDataAddr)
        
            
    #----------------------------------------------------------------------
    def SelectByRawFactor(self,date,stkList,algo,ratio,seq):
        """"""
        ...
        port = {}
        for stk in stkList:
            v = self.getFctVal. Calc(date,180,stk,algo)
            if v!=None:
                if v[0]!=None:
                    stkFctVal[stk] = v[0]
    ...
    #----------------------------------------------------------------------
    def SelectByZScore(self,date,stkList,algo,ratio,seq):
        """"""
        numOfStks = len(stkList)
        numSelected = int(ratio*numOfStks)

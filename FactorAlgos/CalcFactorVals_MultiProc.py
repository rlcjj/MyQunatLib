...
class CalcFactorVals(object):
    ...
    def Calc(self,lookupDate,effectiveDays,stkCode,algos):
        ...
        results = []
        for algo in algos:
            results.append((pool.apply_async(_Calc,(algo.__name__,self.conn,acctPeriods,p,s,date,stkCode,))))
        pool.close()
        pool.join()
        for res in results:
            factorVals.append(res.get())
        tm2 = time.time()
        #print "Time consume:{}".format(tm2-tm1)
        return acctPeriods,rptType,factorVals  
    
#----------------------------------------------------------------------
def _Calc(algoName,conn,acctPeriods,p,s,date,stkCode):
    """"""
    exec("import {} as algo".format(algoName))
    cur = conn.cursor()
